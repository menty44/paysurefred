...
import paygate.objects.UsergroupFacade;
...
public class UsergroupController implements Serializable {
    ...
    @EJB
    private paygate.objects.UsergroupFacade ejbFacade;
    ...
    public String update() {
        try {
            current.setModified(Calendar.getInstance().getTime());
    ...
    public SelectItem[] getItemsAvailableSelectOne() {
        List<Usergroup> usergroupList = ejbFacade.findAll();
        SelectItem[] usergroups = new SelectItem[usergroupList.size()];
        Iterator i = usergroupList.iterator();
        int x = 0;
        while (i.hasNext()) {
            Usergroup usergroup = (Usergroup) i.next();
            usergroups[x++] = new SelectItem(usergroup, usergroup.getName());
        }
        return usergroups;
